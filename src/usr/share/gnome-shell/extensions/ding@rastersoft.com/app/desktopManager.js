...
var DesktopManager = class {
  constructor(mainApp, dbusManager, desktopList, codePath, asDesktop, primaryIndex) {
    ...
    try {
      ...
      this._errorWindow = new ShowErrorPopup.ShowErrorPopup(_('Nautilus File Manager not found'), _('The Nautilus File Manager is mandatory to work with Desktop Icons NG.'), true);
    }
  ...
  updateGridWindows(newdesktoplist) {
    ...
    if (newdesktoplist.length == this._desktopList.length) {
      ...
      for (let index = 0; index < newdesktoplist.length; index++) {
        ...
        if (area.x != area2.x || area.y != area2.y || area.width != area2.width || area.height != area2.height || area.scaleFactor !== area2.scaleFactor || area.monitorIndex != area2.monitorIndex) {
          gridschanged.push(index);
          continue;
        }
        if (area.marginTop != area2.marginTop || area.marginBottom != area2.marginBottom || area.marginLeft != area2.marginLeft || area.marginRight != area2.marginRight) {
  ...
  _configureSelectionColor() {
    ...
    try {
      if (this._accentColorsAvailable) {
        ...
        if (exists) this.selectColor = color;
        else throw new Error('Style Context does not provide accent_bg_color');
      }
    } catch (e) {
    ...
    try {
      ...
      this._cssColorProviderSelection.load_from_data(cssColorDefinition, gsizeLength);
    }
    Gtk.StyleContext.add_provider_for_display(Gdk.Display.get_default(), this._cssColorProviderSelection, Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION);
  }
  ...
  onDragDataReceived(dropInfo, xDestination, yDestination, forceMove) {
    ...
    switch (dropInfo.mimetype) {
      case Enums.DndTargetInfo.DING_ICON_LIST:
        if (dropInfo.filelist.length == 0) return;
  ...
  detectURLorText(fileList, dropCoordinates) {
    ...
    function isValidURL(str) {
      ...
      if (['http', 'https', 'ftp', 'rtsp', 'mms'].includes(scheme.toLowerCase())) return ok;
      else return false;
    }
  ...
  hidePopup() {
    if (this._popupCounter > 0) this._popupCounter--;
    else console.log('Mismatched hidePopup() and showPopup() calls');
  }
  ...
  onKeyRelease(keyval, keycode, state) {
    if (this._popupCounter != 0) return false;

    const isCtrl = (state & Gdk.ModifierType.CONTROL_MASK) != 0;
    const isShift = (state & Gdk.ModifierType.SHIFT_MASK) != 0;

    if (keyval == Gdk.KEY_Left || keyval == Gdk.KEY_Right || keyval == Gdk.KEY_Up || keyval == Gdk.KEY_Down) {
      ...
      if (this.searchString != '') {
        ...
        if (found) {
          if (this.getNumberOfSelectedItems() >= 1 && !this.keypressTimeoutID) {
            let windowError = new ShowErrorPopup.ShowErrorPopup(_('Clear Current Selection before New Search'), null, true);
  ...
  _refreshSearchTimeout() {
    ...
    if (Prefs.a11YKeyboard) {
      ...
      if (Prefs.a11YKeyboard.get_boolean('stickykeys-enable') || Prefs.a11YKeyboard.get_boolean('slowkeys-enable') || Prefs.a11YKeyboard.get_boolean('bouncekeys-enable') || Prefs.a11YKeyboard.get_boolean('mousekeys-enable')) {
        return;
      }
    }
  ...
  _doReadAsync() {
    ...
    return new Promise((resolve, reject) => {
      this._desktopDir.enumerate_children_async(Enums.DEFAULT_ATTRIBUTES, Gio.FileQueryInfoFlags.NONE, GLib.PRIORITY_DEFAULT, this._desktopEnumerateCancellable, (source, result) => {
        this._desktopEnumerateCancellable = null;
        try {
          ...
          for (let [newFolder, extras] of DesktopIconsUtil.getExtraFolders()) {
            try {
              fileList.push(new FileItem.FileItem(this, newFolder, newFolder.query_info(Enums.DEFAULT_ATTRIBUTES, Gio.FileQueryInfoFlags.NONE, null), extras, null));
            } catch (e) {
          ...
          while ((info = fileEnum.next_file(null))) {
            let fileItem = new FileItem.FileItem(this, fileEnum.get_child(info), info, Enums.FileType.NONE, null);
          ...
          for (let [newFolder, extras, volume] of DesktopIconsUtil.getMounts(this._volumeMonitor)) {
            try {
              fileList.push(new FileItem.FileItem(this, newFolder, newFolder.query_info(Enums.DEFAULT_ATTRIBUTES, Gio.FileQueryInfoFlags.NONE, null), extras, volume));
            } catch (e) {
  ...
  _updateWritableByOthers() {
    let info = this._desktopDir.query_info(Gio.FILE_ATTRIBUTE_UNIX_MODE, Gio.FileQueryInfoFlags.NONE, null);
  ...
  doTrash() {
    const selection = this._fileList.filter((i) => (i.isSelected || i.isKeyboardSelected) && !i.isSpecial).map((i) => i.file.get_uri());
  ...
  doDeletePermanently() {
    const toDelete = this._fileList.filter((i) => (i.isSelected || i.isKeyboardSelected) && !i.isSpecial).map((i) => i.file.get_uri());
  ...
  _makeStackTopMarkerFolder(type, list) {
    ...
    let fileItem = new stackItem.stackItem(this, stackAttribute, type, Enums.FileType.STACK_TOP);
    list.push(fileItem);
  }
  ...
  _sortByKindByName(fileList) {
    ...
    function byKindByName(a, b) {
      return a.attributeContentType.localeCompare(b.attributeContentType) || a._label.get_text().localeCompare(b._label.get_text(), { sensitivity: 'accent', numeric: 'true', localeMatcher: 'lookup' });
    }
