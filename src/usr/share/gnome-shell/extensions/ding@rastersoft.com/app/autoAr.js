...
var AutoAr = class {
  ...
  checkAutoAr() {
    if (GnomeAutoar === null) {
      this._desktopManager.dbusManager.doNotify(_('AutoAr is not installed'), _('To be able to work with compressed files, install file-roller and/or gir-1.2-gnomeAutoAr'));
    }
  ...
  runToolAsync(autoArTool, cancellable) {
    return new Promise((resolve, reject) => {
      ...
      connections.push(
        autoArTool.connect('error', (w, error) => {
          connections.forEach((c) => autoArTool.disconnect(c));
          reject(error);
        })
      );
  ...
  extractFile(fileName) {
    ...
    this._password = null;
    doExtract.doExtractFile(fullPath, folder, folderName).catch((e) => console.error(e));
  }
  ...
  compressFiles(fileList, outputFile, format, filter, password = null) {
    ...
    doCompress.doCompressFiles(fileList, outputFile, format, filter, password).catch((e) => console.error(e));
  }
  ...
  addProgress(progressElement, message) {
    ...
    if (this._progressElements == 0) {
      this._inhibitCookie = this._desktopManager.mainApp.inhibit(null, Gtk.ApplicationInhibitFlags.LOGOUT | Gtk.ApplicationInhibitFlags.SUSPEND, message);
    }
...
const progressDialog = class {
  ...
  async _cleanupFile(file, cancellable) {
    ...
    this._processLabel.set_label(_("Removing partial file '${outputFile}'").replace('${outputFile}', file.get_basename()));
  ...
  async doExtractFile(fullPath, folder, folderName, counter = 1) {
    ...
    try {
      ...
      try {
        await folder.set_attributes_async_promise(info, Gio.FileQueryInfoFlags.NONE, GLib.PRIORITY_DEFAULT, this._cancellable);
      } catch (e) {
    ...
    try {
      ...
      this._autoAr.notify(_('Extraction completed'), _("Extracting '${fullPathFile}' has been completed.").replace('${fullPathFile}', fullPathFile.get_basename()));
    } catch (e) {
      if (e.matches(Gio.IOErrorEnum, Gio.IOErrorEnum.CANCELLED)) {
        this._cancellable = new Gio.Cancellable();
        await this._cleanupFile(folder, this._cancellable);
        this._autoAr.notify(_('Extraction cancelled'), _("Extracting '${fullPathFile}' has been cancelled by the user.").replace('${fullPathFile}', fullPathFile.get_basename()));
      } else {
  ...
  async doCompressFiles(fileList, outputFile, format, filter, password = null) {
    ...
    try {
      ...
      this._autoAr.notify(_('Compression completed'), _("Compressing files into '${outputFile}' has been completed.").replace('${outputFile}', output.get_basename()));
    } catch (e) {
      if (e.matches(Gio.IOErrorEnum, Gio.IOErrorEnum.EXISTS)) {
        this._autoAr.notify(_('Cancelled compression'), _("The output file '${outputFile}' already exists.").replace('${outputFile}', output.get_basename()));
      } else {
        this._cancellable = new Gio.Cancellable();
        await this._cleanupFile(output, this._cancellable);
        if (e.matches(Gio.IOErrorEnum, Gio.IOErrorEnum.CANCELLED)) {
          this._autoAr.notify(_('Cancelled compression'), _("Compressing files into '${outputFile}' has been cancelled by the user.").replace('${outputFile}', output.get_basename()));
        } else {
