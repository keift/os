...
export default class DING extends Extension {
  ...
  innerEnable() {
     ...
     */
    this.data.scaleFactorId = St.ThemeContext.get_for_stage(global.stage).connect('notify::scale-factor', () => this.updateDesktopGeometry());
    ...
    this.data.dbusConnectionId = Gio.bus_own_name(
      Gio.BusType.SESSION,
      ...
      (connection, name) => {
        ...
        this.data.dbusConnectionGroupId = this.data.dbusConnection.export_action_group('/com/rastersoft/dingextension/control', this.data.actionGroup);
        this.launchDesktop();
      },
      null
    );
