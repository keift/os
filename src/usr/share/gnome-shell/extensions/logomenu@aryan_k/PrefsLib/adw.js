...
const IconGrid = GObject.registerClass(
  class LogoMenuIconGrid extends Gtk.FlowBox {
    _init() {
      super._init({
        row_spacing: 10,
        column_spacing: 10,
...
export const LogoMenuIconsPage = GObject.registerClass(
  class LogoMenuIconsWidget extends Adw.PreferencesPage {
    _init(settings) {
      super._init();
      ...
      if (this._settings.get_string('custom-icon-path')) customIconPreview.set_from_file(this._settings.get_string('custom-icon-path'));
...
export const LogoMenuOptionsPage = GObject.registerClass(
  class LogoMenuOptionsWidget extends Adw.PreferencesPage {
    _init(settings) {
      super._init();
      this._settings = settings;
...
export const AboutPage = GObject.registerClass(
  class LogoMenuAboutPage extends Adw.PreferencesPage {
    _init(metadata) {
      ...
      let releaseVersion;
      if (metadata['version-name']) releaseVersion = metadata['version-name'];
      else releaseVersion = 'unknown';
      logoMenuVersionRow.add_suffix(
        new Gtk.Label({
