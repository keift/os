...
export class Registry {
  ...
  writeToFile(registry) {
    ...
    file.replace_async(null, false, Gio.FileCreateFlags.NONE, GLib.PRIORITY_DEFAULT, null, (obj, res) => {
      ...
      stream.write_bytes_async(contents, GLib.PRIORITY_DEFAULT, null, (w_obj, w_res) => {
  ...
  async read() {
    return new Promise((resolve) => {
      if (GLib.file_test(this.REGISTRY_PATH, FileTest.EXISTS)) {
        ...
        file.query_info_async('*', FileQueryInfoFlags.NONE, GLib.PRIORITY_DEFAULT, null, (src, res) => {
...
export class ClipboardEntry {
  ...
  static __isText(mimetype) {
    return mimetype.startsWith('text/') || mimetype === 'STRING' || mimetype === 'UTF8_STRING';
  }
        ...
        bytes = await new Promise((resolve, reject) =>
          file.load_contents_async(null, (obj, res) => {
            ...
            if (success) {
              ...
              reject(new Error('Clipboard Indicator: could not read image file from cache'));
            }
          })
        );
      }
  ...
  constructor(mimetype, bytes, favorite) {
    ...
    return [...this.#bytes].map((x) => x.toString(16).padStart(2, '0')).join('');
  }
