...
const ClipboardIndicator = GObject.registerClass(
  ...
  class ClipboardIndicator extends PanelMenu.Button {
    ...
    async _buildMenu() {
      ...
      this.searchEntry.get_clutter_text().connect('text-changed', this._onSearchTextChanged.bind(this));
      ...
      // Private mode switch
      this.privateModeMenuItem = new PopupMenu.PopupSwitchMenuItem(_('Private mode'), PRIVATEMODE, { reactive: true });
      this.privateModeMenuItem.connect('toggled', this._onPrivateModeSwitch.bind(this));
    ...
    #hideElements() {
      ...
      if (this.clearMenuItem?.actor && this.menu.box.contains(this.clearMenuItem.actor)) this.menu.box.remove_child(this.clearMenuItem.actor);
      if (this.settingsMenuItem?.actor && this.menu.box.contains(this.settingsMenuItem.actor)) this.menu.box.remove_child(this.settingsMenuItem.actor);
    ...
    #showElements() {
      ...
      if (SHOW_SEARCH_BAR && !PRIVATEMODE) {
        if (!this.menu.box.contains(this._entryItem)) this.menu.box.insert_child_at_index(this._entryItem, 0);
      } else {
        if (this.menu.box.contains(this._entryItem)) this.menu.box.remove_child(this._entryItem);
      }
      ...
      if (this.clipItemsRadioGroup.length > 0 && this.historySection._getMenuItems().length > 0 && !PRIVATEMODE && (SHOW_PRIVATE_MODE || SHOW_SETTINGS_BUTTON || SHOW_CLEAR_HISTORY_BUTTON)) {
        if (!this.menu.box.contains(this.historySeparator)) this.menu.box.insert_child_above(this.historySeparator, this.scrollViewMenuSection.actor);
      ...
      if (this.clipItemsRadioGroup.length === 0) {
        if (!this.menu.box.contains(this.emptyStateSection)) this.#renderEmptyState();
        // Re-append toggled buttons after the empty state
        if (this.menu.box.contains(this.settingsMenuItem?.actor)) this.menu.box.remove_child(this.settingsMenuItem.actor);

        let index = this.menu.box.get_n_children(); // append after empty state
        if (SHOW_SETTINGS_BUTTON && this.settingsMenuItem) this.menu.box.insert_child_at_index(this.settingsMenuItem.actor, index++);
        return;
      }

      // Re-append toggled buttons at end in fixed order
      if (this.menu.box.contains(this.settingsMenuItem?.actor)) this.menu.box.remove_child(this.settingsMenuItem.actor);
      if (this.menu.box.contains(this.clearMenuItem?.actor)) this.menu.box.remove_child(this.clearMenuItem.actor);

      let index = this.menu.box.get_n_children(); // append
      if (SHOW_SETTINGS_BUTTON && this.settingsMenuItem) this.menu.box.insert_child_at_index(this.settingsMenuItem.actor, index++);
      if (SHOW_CLEAR_HISTORY_BUTTON && this.clearMenuItem && !PRIVATEMODE) this.menu.box.insert_child_at_index(this.clearMenuItem.actor, index++);
    }
    ...
    _truncate(string, length) {
      ...
      let chars = [...shortened];
      if (chars.length > length) shortened = chars.slice(0, length - 1).join('') + '...';
    ...
    _addEntry(entry, autoSelect, autoSetClip) {
      ...
      menuItem.connect('key-focus-in', () => {
        const viewToScroll = menuItem.entry.isFavorite() ? this.favoritesScrollView : this.historyScrollView;
      ...
      menuItem.favoritePressId = icofavBtn.connect('clicked', () => this._favoriteToggle(menuItem));
      ...
      menuItem.pasteBtn.connect('clicked', () => this.#pasteItem(menuItem));
      ...
      menuItem.deletePressId = icoBtn.connect('clicked', () => (menuItem.entry.isFavorite() ? (CONFIRM_ON_PINNED_DELETE ? this._confirmRemovePinnedEntry(menuItem) : this._removeEntry(menuItem, 'delete')) : this._removeEntry(menuItem, 'delete')));
    ...
    _clearHistory(invokedAutomatically = false) {
      ...
      if (NOTIFY_ON_CLEAR) {
        const message = invokedAutomatically ? _('Clipboard history cleared automatically') : _('Clipboard history cleared');
    ...
    _removeOldestEntries() {
      let clipItemsRadioGroupNoFavorite = this.clipItemsRadioGroup.filter((item) => item.entry.isFavorite() === false);
      ...
      while (clipItemsRadioGroupNoFavorite.length > MAX_REGISTRY_LENGTH) {
        ...
        clipItemsRadioGroupNoFavorite = this.clipItemsRadioGroup.filter((item) => item.entry.isFavorite() === false);
      }
    ...
    _updateCache() {
      const entries = this.clipItemsRadioGroup.map((menuItem) => menuItem.entry).filter((entry) => CACHE_ONLY_FAVORITE == false || entry.isFavorite());
    ...
    _setupHistoryIntervalClearing() {
      ...
      this._intervalSettingChangedId = this.extension.settings.connect(`changed::${PrefsFields.CLEAR_HISTORY_INTERVAL}`, this._onHistoryIntervalClearSettingsChanged.bind(this));
      this._intervalToggleChangedId = this.extension.settings.connect(`changed::${PrefsFields.CLEAR_HISTORY_ON_INTERVAL}`, this._onHistoryIntervalClearSettingsChanged.bind(this));
    ...
    _loadSettings() {
      this._settingsChangedId = this.extension.settings.connect('changed', this._onSettingsChange.bind(this));
    ...
    _bindShortcut(name, cb) {
      Main.wm.addKeybinding(name, this.extension.settings, Meta.KeyBindingFlags.NONE, Shell.ActionMode.ALL, cb.bind(this));
    ...
    #pasteItem(menuItem) {
      ...
      this._pastingKeypressTimeout = setTimeout(() => {
        ...
        this._pastingResetTimeout = setTimeout(() => {
          ...
          if (currentlySelected && currentlySelected.entry) this.#updateClipboard(currentlySelected.entry);
        }, 50);
      }, 50);
    }
    ...
    async #getClipboardContent() {
      const mimetypes = ['text/plain;charset=utf-8', 'UTF8_STRING', 'text/plain', 'STRING', 'image/gif', 'image/png', 'image/jpg', 'image/jpeg', 'image/webp', 'image/svg+xml', 'text/html'];
